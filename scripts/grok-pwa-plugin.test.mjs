...
test("web manifest prefers the site title over the host slug", () => {
  const manifest = JSON.parse(
    renderWebManifest("pixel-bloom-monarch-arch.grok.me", { title: "Spoonful" }),
  );
  assert.equal(manifest.name, "Spoonful");
  assert.equal(manifest.short_name, "Spoonful");
  assert.equal(manifest.display, "standalone");
  assert.equal(manifest.theme_color, "#e24a12");
});

test("skips Created-with-Grok banner on a non-grok public host", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "spoonful-kitchen.vercel.app",
    projectId: "proj-123",
    creator: "@alice",
    creatorId: "42",
  });
...
test("keeps Created-with-Grok banner on a grok.me host", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "pixel-bloom-monarch-arch.grok.me",
    projectId: "proj-123",
  });
  assert.match(out, /grok-app-builder\/extensions\.js/);
  assert.match(out, /grok-project-id/);
});

test("does not inject grok manifest when a manifest link already exists", () => {
  const out = injectGrokPwaHead(

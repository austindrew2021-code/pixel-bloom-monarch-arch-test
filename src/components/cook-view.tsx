...
export function CookView({ meal, onClose }: { meal: PlannedMeal; onClose: () => void }) {
  ...
  const handsFreeCook = useSpoonful((s) => s.handsFreeCook);
  const setHandsFreeCook = useSpoonful((s) => s.setHandsFreeCook);
  ...
  useEffect(() => {
    ...
    if (useSpoonful.getState().handsFreeCook && step < steps.length - 1) {
      setStep((s) => s + 1);
    }
  ...
  const skipSpeak = useRef(true);
  useEffect(() => {
    if (!handsFreeCook) return;
    if (skipSpeak.current) {
      skipSpeak.current = false;
      return;
    }
    speak();
    // speak is recreated each render; only fire on step
  ...
  useEffect(() => {
    ...
    const SR =
      ...
      (window as unknown as { SpeechRecognition?: new () => VoiceRec }).SpeechRecognition;
    if (!SR) return;
    const rec = new SR();
    rec.continuous = true;
    ...
    rec.onresult = (ev) => {
      ...
      if (/next|continue|forward|done/.test(said)) {
        stopSpeak();
        setStep((s) => Math.min(steps.length - 1, s + 1));
      } else if (/back|previous/.test(said)) {
        stopSpeak();
        setStep((s) => Math.max(0, s - 1));
      } else if (/repeat|again/.test(said)) {
        speak();
      }
    };
    try {
        ...
        {items.map((ing, i) => {
          ...
          return (
            <li key={`${ing.name}-${i}`}>
              ...
              {localSubs(ing.name)[0] ? (
                <p className="pl-9 text-xs text-muted-foreground">
                  Out? Use {localSubs(ing.name)[0]!.name} — {localSubs(ing.name)[0]!.note}
                </p>
              ) : null}
            </li>
          );
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background text-foreground">
      ...
      <footer className="flex flex-col gap-2 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-2">
        <button
          ...
          onClick={() => setHandsFreeCook(!handsFreeCook)}
        >
          {handsFreeCook
            ? "Hands-free on · reads the next step. Say next, back, or repeat."
            : "Hands-free off · tap to keep cooking while your hands are messy"}
        </button>

...
export function RecipesView({ onOpenStore }: { onOpenStore: () => void }) {
  ...
  const clipIntoTonight = useSpoonful((s) => s.clipIntoTonight);
  const lookupRemaining = useSpoonful((s) => s.lookupRemaining);
  ...
  return (
    ...
    <div className="mx-auto max-w-2xl overflow-x-clip px-4 pb-36 pt-4" inert={Boolean(cooking)}>
      ...
      {clipOpen ? (
        <div className="mt-2 rounded-3xl bg-card p-3 shadow-[var(--shadow-border)]">
          <p className="mb-2 text-xs text-muted-foreground">
            {hasPlus
              ? `${lookupRemaining()} clips left this week.`
              : `${lookupRemaining()} free clips this week. Kitchen+ opens more.`}
          </p>
          <textarea
            value={clipText}
            onChange={(e) => setClipText(e.target.value)}
            rows={5}
            placeholder={"Name on the first line.\nThen ingredients, one per line."}
            className="w-full rounded-2xl bg-background p-3 text-sm leading-relaxed shadow-[var(--shadow-border)] outline-none"
          />
          <Button
            ...
            onClick={() => {
              const left = lookupRemaining();
              if (left <= 0) {
                onOpenStore();
                toast("Free kitchens get 3 clips a week. Kitchen+ opens more.");
                return;
              }
              const name = clipIntoTonight(clipText);
              toast(name ? `Tonight is ${name}` : "Lookup limit for this week. Kitchen+ opens more.");
              if (name) setClipText("");
            }}
          >
            Plate tonight from this
          </Button>
        </div>
      ) : null}

...
export function FitView({ onOpenStore }: { onOpenStore?: () => void }) {
  ...
  const eaten = nutritionForDate(meals, today, snacks, portionMultByDate, cookedDates);
  ...
  const weekProtein = weekDatesList.reduce((sum, d) => sum + nutritionForDate(meals, d, snacks, portionMultByDate, cookedDates).protein, 0);
  ...
  return (
    <div className="mx-auto max-w-2xl overflow-x-clip px-4 pb-36 pt-4">
      ...
      <button type="button" className="hud-btn hud-btn-on mt-3 flex h-12 w-full items-center justify-center gap-2 text-sm font-semibold" onClick={() => setTab("train")}>
      ...
      <section className="mt-5 rounded-3xl bg-spark p-4 text-spark-foreground" data-tour="fuel-now">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          <LiveStat label="Left" value={`${Math.round(fuel.remaining.cal)}`} hint="kcal" testId="fuel-left" />
          <LiveStat label="Protein" value={`${Math.round(fuel.remaining.protein)}`} hint="g" testId="fuel-left-protein" />
      ...
      <section className="mt-5 rounded-3xl bg-card p-4 shadow-[var(--shadow-border)]">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground" data-testid="body-age-line">
            ...
            {ageNeedsTrainingNote(body.age) ? (
              <p className="mt-2 text-xs leading-relaxed text-muted-foreground" data-testid="age-why">
                {AGE_TRAINING_NOTE}
              </p>
            ) : null}
            {/*
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="text-xs text-muted-foreground">
                ...
                <Input
                  className="mt-1"
                  inputMode="numeric"
                  data-testid="body-age"
                  defaultValue={String(body.age)}
                  key={body.age}
                  onBlur={(e) => setBody({ age: clampAge(Number(e.target.value) || 34) })}
                />
              </label>
              <label className="text-xs text-muted-foreground">
...
function LiveStat({
  ...
  return (
    <div className="min-w-0 overflow-hidden" data-testid={testId}>

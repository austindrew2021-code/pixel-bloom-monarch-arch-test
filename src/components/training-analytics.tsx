...
export function TrainingAnalytics({ onOpenStore }: { onOpenStore: () => void }) {
  ...
  if (!hasKitchenTable) {
    return (
      <button
        type="button"
        onClick={onOpenStore}
        className="hud-panel mt-4 w-full p-4 text-left"
      >
  ...
  return (
    <section className="hud-panel mt-4 p-4">
      <p className="hud-kicker">Training analytics</p>
      ...
      {lifts.length ? (
        <div className="mt-4">
          ...
          <ul className="mt-2 divide-y divide-foreground/15">
            {lifts.map((lift) => (

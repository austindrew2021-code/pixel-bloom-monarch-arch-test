import { SpinningMoon, Starfield } from "@/components/sky-art";
...
function NebulaArt() {
  return (
    <svg className="theme-art__svg" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice" aria-hidden>
      ...
      {/* Stars hold still in the gas layer. Twinkles live in NebulaTwinkles above the veil. */}
      <g fill="#ffffff">
        {STARS.map(([x, y, r, o], i) => (
          <circle key={i} cx={`${x}%`} cy={`${y}%`} r={r} opacity={o * 0.55} />
        ))}
      </g>
    </svg>
  );
...
function Tumbleweeds() {
  return (
    <div className="theme-art__tumbles">
      <img className="theme-art__tumble" src="/themes/tumbleweed.png" alt="" />
      <img className="theme-art__tumble theme-art__tumble--b" src="/themes/tumbleweed.png" alt="" />
      <img className="theme-art__tumble theme-art__tumble--c" src="/themes/tumbleweed.png" alt="" />
    </div>
  );
}

const MAPLE =
  "M12 1 C13 5 16 7 19 8 C16 9 16 12 18 15 C14 14 13 17 14 20 C12 18 11 21 12 23 C10 20 8 18 6 20 C7 16 5 14 2 15 C5 11 4 8 2 7 C6 7 8 4 9 1 C10 4 11 4 12 1Z";
const OAK =
  "M12 2 C14 4 18 5 19 8 C16 8 17 12 20 13 C16 13 16 17 19 19 C15 18 14 22 16 24 C13 22 11 24 12 26 C10 23 8 22 6 24 C8 20 6 18 3 19 C6 16 5 13 2 13 C5 11 4 8 2 7 C6 7 8 4 10 2 C11 4 12 4 12 2Z";

const LEAVES: { x: number; delay: number; dur: number; size: number; color: string; drift: number; oak?: boolean }[] = [
  { x: 4, delay: 0, dur: 14, size: 22, color: "#c44b1a", drift: 22 },
  { x: 14, delay: 1.8, dur: 17, size: 16, color: "#d9782a", drift: -16, oak: true },
  { x: 24, delay: 3.2, dur: 15, size: 20, color: "#a83412", drift: 18 },
  { x: 34, delay: 0.7, dur: 19, size: 14, color: "#e0a030", drift: -12 },
  { x: 44, delay: 5.4, dur: 13, size: 24, color: "#c44b1a", drift: 26, oak: true },
  { x: 54, delay: 2.1, dur: 18, size: 17, color: "#8a2a10", drift: -20 },
  { x: 64, delay: 7.6, dur: 16, size: 19, color: "#d9782a", drift: 14 },
  { x: 74, delay: 4.4, dur: 20, size: 15, color: "#b84a18", drift: -18, oak: true },
  { x: 84, delay: 6.1, dur: 14, size: 21, color: "#e08a28", drift: 12 },
  { x: 92, delay: 9.2, dur: 17, size: 13, color: "#c44b1a", drift: -10 },
  { x: 8, delay: 11, dur: 15, size: 18, color: "#d9782a", drift: 16 },
  { x: 28, delay: 8.4, dur: 21, size: 16, color: "#a83412", drift: -14, oak: true },
  { x: 48, delay: 12.2, dur: 14, size: 23, color: "#e0a030", drift: 20 },
  { x: 68, delay: 10.1, dur: 18, size: 14, color: "#c44b1a", drift: -22 },
  { x: 88, delay: 13.5, dur: 16, size: 19, color: "#8a2a10", drift: 11, oak: true },
  { x: 18, delay: 15, dur: 19, size: 15, color: "#d9782a", drift: -8 },
  { x: 58, delay: 6.8, dur: 13, size: 20, color: "#b84a18", drift: 24 },
  { x: 38, delay: 16.4, dur: 17, size: 17, color: "#e08a28", drift: -15 },
];
  ...
  return (
    <svg className="theme-art__svg" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice" aria-hidden>
      {LEAVES.map((leaf, i) => (
        <g
          key={i}
          className="theme-art__fall"
          style={
            {
              "--fall-delay": `${leaf.delay}s`,
              "--fall-dur": `${leaf.dur}s`,
              "--fall-drift": `${leaf.drift}px`,
            } as CSSProperties
          }
        >
          <g transform={`translate(${(leaf.x / 100) * 400} -30)`}>
            <path d={leaf.oak ? OAK : MAPLE} fill={leaf.color} opacity="0.92" transform={`scale(${leaf.size / 12})`} />
          </g>
        </g>
      ))}
    </svg>
...
const PETALS: { x: number; delay: number; dur: number; size: number; color: string; drift: number }[] = [
  { x: 6, delay: 0, dur: 16, size: 12, color: "#f4b6c8", drift: 18 },
  { x: 18, delay: 2.2, dur: 20, size: 9, color: "#f7d3de", drift: -12 },
  { x: 30, delay: 4.6, dur: 15, size: 14, color: "#e88aa8", drift: 22 },
  { x: 44, delay: 1.1, dur: 21, size: 10, color: "#f4b6c8", drift: -16 },
  { x: 56, delay: 7.2, dur: 17, size: 13, color: "#d97a98", drift: 14 },
  { x: 70, delay: 3.4, dur: 19, size: 9, color: "#f7d3de", drift: -20 },
  { x: 82, delay: 9.1, dur: 16, size: 12, color: "#e88aa8", drift: 10 },
  { x: 92, delay: 5.8, dur: 22, size: 11, color: "#f4b6c8", drift: -14 },
  { x: 12, delay: 11.4, dur: 18, size: 10, color: "#d97a98", drift: 16 },
  { x: 38, delay: 8.6, dur: 14, size: 13, color: "#f7d3de", drift: -9 },
  { x: 64, delay: 13, dur: 20, size: 9, color: "#e88aa8", drift: 12 },
  { x: 88, delay: 10.2, dur: 15, size: 12, color: "#f4b6c8", drift: -18 },
];
...
const FLIES: { x: number; y: number; delay: number }[] = [
  { x: 14, y: 18, delay: 0 },
  { x: 28, y: 26, delay: 1.1 },
  { x: 46, y: 14, delay: 2.4 },
  { x: 62, y: 22, delay: 0.6 },
  ...
  return (
    <svg className="theme-art__svg" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice" aria-hidden>
      {FLIES.map((f, i) => (
        <g
          key={i}
          className="theme-art__fly"
          style={{ "--fly-delay": `${f.delay}s` } as CSSProperties}
        >
          <circle cx={`${f.x}%`} cy={`${f.y}%`} r="2.4" fill="#ffe08a" opacity="0.9" />
          <circle cx={`${f.x}%`} cy={`${f.y}%`} r="7" fill="#ffe08a" opacity="0.22" />
        </g>
      ))}
    </svg>
  );
...
const FLAKES: { x: number; delay: number; dur: number; size: number; drift: number }[] = [
  { x: 6, delay: 0, dur: 16, size: 3.6, drift: 12 },
  { x: 16, delay: 1.4, dur: 20, size: 2.4, drift: -9 },
  { x: 26, delay: 3.8, dur: 14, size: 4.2, drift: 16 },
  { x: 38, delay: 0.8, dur: 22, size: 2.8, drift: -14 },
  { x: 48, delay: 5.6, dur: 17, size: 3.4, drift: 10 },
  { x: 58, delay: 2.6, dur: 19, size: 2.2, drift: -7 },
  { x: 68, delay: 7.1, dur: 15, size: 3.8, drift: 13 },
  { x: 78, delay: 4.2, dur: 21, size: 2.6, drift: -11 },
  { x: 88, delay: 9, dur: 18, size: 3.1, drift: 8 },
  { x: 94, delay: 6.4, dur: 16, size: 2.5, drift: -15 },
  { x: 10, delay: 11, dur: 23, size: 2.9, drift: 9 },
  { x: 22, delay: 8.2, dur: 14, size: 3.5, drift: -10 },
  { x: 42, delay: 12.4, dur: 20, size: 2.3, drift: 14 },
  { x: 62, delay: 10.6, dur: 17, size: 3.9, drift: -8 },
  { x: 82, delay: 13.8, dur: 19, size: 2.7, drift: 11 },
  { x: 34, delay: 15, dur: 15, size: 3.2, drift: -13 },
];
  ...
  return (
    <svg className="theme-art__svg" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice" aria-hidden>
      {FLAKES.map((f, i) => (
        <g
          key={i}
          className="theme-art__fall theme-art__fall--soft"
          style={
            {
        ...
        >
          <circle cx={`${f.x}%`} cy="-4%" r={f.size} fill="#d7e8f5" opacity="0.85" />
        </g>
      ))}
    </svg>
  );
...
function AetherHud() {
  return (
    <svg className="theme-art__svg" viewBox="0 0 400 800" preserveAspectRatio="xMidYMid slice" aria-hidden>
      <g fill="none" stroke="#3dffd0" opacity="0.28">
        <rect x="18" y="18" width="28" height="28" />
        <rect x="354" y="18" width="28" height="28" />
        <rect x="18" y="754" width="28" height="28" />
        <rect x="354" y="754" width="28" height="28" />
        <path d="M18 32 H8 M32 18 V8 M382 32 H394 M368 18 V8 M18 768 H8 M32 782 V794 M382 768 H394 M368 782 V794" />
      </g>
      <g className="theme-art__spin theme-art__spin--cw-slow" style={{ transformOrigin: "320px 120px" }} opacity="0.35">
        <circle cx="320" cy="120" r="54" fill="none" stroke="#3dffd0" strokeWidth="0.8" strokeDasharray="4 7" />
        <circle cx="320" cy="120" r="34" fill="none" stroke="#ffb45a" strokeWidth="0.6" strokeDasharray="2 10" />
        <circle cx="320" cy="120" r="4" fill="#3dffd0" />
      </g>
      <g opacity="0.2" stroke="#3dffd0" fill="none">
        <path d="M40 640 H140 M40 640 V700" strokeWidth="0.7" />
        <path d="M260 720 H360 M360 720 V660" strokeWidth="0.7" />
      </g>
    </svg>
  );
}
...
export function ThemeOrnament({ theme }: { theme: ThemeId }) {
  const art = themeById(theme).art;
  const photo = Boolean(art);
  const deepSky = theme === "midnight";
  if (theme !== "brass" && theme !== "west" && !photo && !deepSky) return null;
  return (
    <div className="theme-art" aria-hidden>
      {art ? <img className="theme-art__photo theme-art__photo--live" src={art} alt="" /> : null}
      {theme === "brass" ? <BrassArt /> : null}
      {deepSky ? (
        <>
          <Starfield seed={3} count={130} />
          <SpinningMoon className="theme-art__moon--far" />
        </>
      ) : null}
      {theme === "nebula" ? (
        <>
      ...
      {theme === "west" ? <Tumbleweeds /> : null}
      {theme === "autumn" ? <FallingLeaves /> : null}
      {theme === "spring" ? <Petals /> : null}
      {theme === "summer" ? <Fireflies /> : null}
      {theme === "winter" ? <Snow /> : null}
    </div>
  );
}

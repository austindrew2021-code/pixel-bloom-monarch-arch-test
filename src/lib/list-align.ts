 ...
 */
/**
 * A recipe that offers either of two interchangeable staples needs one of them.
 * "One tablespoonful of melted lard or butter" is Abby Fisher giving the cook a
 * choice, and it bought her corn bread a pound of butter beside the lard
 * already on the list; "a little flour or two tablespoonfuls of cracker crumbs"
 * did the same to an oyster stew. Keep the first and drop the alternative.
 */
const STAPLE =
  "lard|butter|shortening|oil|drippings|suet|bacon grease|schmaltz|flour|cornstarch|corn-?starch|cracker crumbs|bread crumbs|crumbs|milk|cream|water|stock|broth";
const EITHER_OF = new RegExp(
  `\\b(?:${STAPLE})\\s+or\\s+(?:a little |two tablespoonfuls of |some )?(?:${STAPLE})\\b`,
  "gi",
);

...
export type TabId = "plan" | "recipes" | "snap" | "people" | "shop" | "fit" | "train" | "sauces" | "desserts";
...
type SpoonfulState = {
  ...
  planUpdates: KitchenUpdate[];
  favStore: NearbyStore | null;
  nextGen: boolean;
  portionSync: boolean;
  ...
  favMoves: string[];
  leftoverVault: { id: string; fromDate: string; title: string }[];
  weekBudgetCad: number;
  bagMemory: Record<string, { pickName: string; packs: number; pickCad: number }>;
  cookByDate: Record<string, string>;
  ...
  plateTonightMood: (mood: "tired" | "comfort" | "protein" | "fridge") => Recipe | null;
  plateLeftoversTonight: () => string | null;
  replayLastWeek: () => number;
  setWeekBudget: (cad: number) => void;
  swapDownOverBudget: () => { swapped: number; saved: number };
  rememberBag: (rows: { name: string; pickName: string; packs: number; pickCad: number }[]) => void;
  platePostLift: () => Recipe | null;
  clipIntoTonight: (text: string) => string | null;
  assignCook: (date: string, name: string) => void;
  ...
  replateFromFuel: (dates?: string[], opts?: { notify?: boolean }) => { count: number; names: string[] };
  ...
  addSnack: (input: { date: string; name: string; nutrition: Snack["nutrition"]; unknownMacros?: boolean }) => void;
  ...
  setTrainHud: (mode: "basic" | "advanced") => void;
  ...
  markPlanUpdatesRead: () => void;
  favoriteStore: (store: NearbyStore | null) => void;
  markSnapped: () => void;
};
...
function pingCoach(get: () => SpoonfulState, event: CoachEvent) {
  const s = get();
  ...
  persist(
    (set, get) => ({
      ...
      completeOnboarding: ({ household, prefs, allergies, sample, nextGen, goal, body, locale, country }) => {
        ...
        if (sample && nextGen) get().replateFromFuel(undefined, { notify: false });
      },
      setTab: (tab) => set({ tab }),
      ...
      setNextGen: (nextGen) => {
        set((s) => ({
          nextGen,
          tab: !nextGen && (s.tab === "fit" || s.tab === "train") ? "plan" : nextGen && s.tab === "people" ? "train" : s.tab,
        }));
        if (nextGen) get().ensureProgram();
      },
      ...
      skipNight: (date, kind) => {
        ...
        if (kind === "takeout") pingCoach(get, "ateOut");
      },
      ...
      fillWeek: (overwrite) => {
        ...
        const pool = shuffle(
          allowedPool(unlocked, prefs, allergies, hidden, goalKind).filter((r) => !seatAvoidsName(r.name, get().seats ?? [])),
        );
        ...
        if (get().weekBudgetCad > 0) get().swapDownOverBudget();
        return filled;
      },
      fillFromFuel: () => {
      ...
      surpriseDinner: (date) => {
        ...
        if (nextGen) {
          ...
          const eaten = nutritionForDate(meals, date, snacks, undefined, get().cookedDates);
      ...
      plateTonightMood: (mood) => {
        const date = isoDate();
        const { meals, unlocked, prefs, allergies, hidden, pantry, body, seats, cookedDates } = get();
        const goalKind = tableGoalOf(body, seats ?? []);
        let pool = allowedPool(unlocked, prefs, allergies, hidden, goalKind);
        if (mood === "tired") pool = pool.filter((r) => r.minutes <= 22);
        else if (mood === "comfort") pool = pool.filter((r) => isComfort(r));
        else if (mood === "protein") pool = pool.filter((r) => isHighProtein(r));
        const used = new Set(meals.filter((m) => m.recipeId).map((m) => m.recipeId as string));
        for (const d of cookedDates) {
          const dinner = meals.find((m) => m.date === d && m.slot === "dinner" && m.recipeId);
          if (dinner?.recipeId) used.add(dinner.recipeId);
        }
        const prev = meals.find((m) => m.date === isoDate(addDays(parseISO(`${date}T12:00:00`), -1)) && m.slot === "dinner");
        let pick: Recipe | undefined;
        if (mood === "fridge") {
          if (pantry.length < 2) return null;
          const ideas = mealsFromPantry(
            pantry.map((p) => p.name),
            pool,
            8,
          );
          const hit = ideas.find((row) => row.recipeId && !used.has(row.recipeId));
          pick = hit?.recipeId ? pool.find((r) => r.id === hit.recipeId) : pool.find((r) => r.id === ideas[0]?.recipeId);
        } else {
          pick = pickVaried(shuffle(pool), used, proteinOf(prev), undefined, pantry.map((p) => p.name), false, undefined, goalKind);
        }
        if (!pick) pick = shuffle(pool)[0];
        if (!pick) return null;
        get().assignMeal(date, "dinner", pick.id);
        return pick;
      },
      plateLeftoversTonight: () => {
        const today = isoDate();
        const { meals, cookedDates } = get();
        const past = [...cookedDates].filter((d) => d < today).sort().reverse();
        for (const d of past) {
          const dinner = meals.find((m) => m.date === d && m.slot === "dinner" && !m.skip);
          if (!dinner) continue;
          ...
          get().assignCustom(today, "dinner", {
            id: `leftover-tonight-${d}`,
      ...
      replayLastWeek: () => {
        const { weekStart, meals, cookedDates } = get();
        const prevDates = weekDates(shiftWeek(weekStart, -1));
        const dates = weekDates(weekStart);
        const snapshot = meals;
        let filled = 0;
        let next = meals.slice();
        dates.forEach((date, i) => {
          if (cookedDates.includes(date)) return;
          const existing = next.find((m) => m.date === date && m.slot === "dinner");
          if (existing && (existing.skip || existing.custom || (existing.recipeId && existing.auto !== true))) return;
          const srcDate = prevDates[i];
          const src = meals.find((m) => m.date === srcDate && m.slot === "dinner");
          if (!src || src.skip) return;
          next = next.filter((m) => !(m.date === date && m.slot === "dinner"));
          next.push({
            id: uid(),
            date,
            slot: "dinner",
        ...
        set({ meals: next, undoMeals: snapshot });
        return filled;
      },
      setWeekBudget: (cad) => set({ weekBudgetCad: Math.max(0, Math.round(cad)) }),
      swapDownOverBudget: () => {
        const { weekStart, meals, household, cookedDates, unlocked, prefs, allergies, hidden, body, seats, weekBudgetCad } =
          get();
        if (weekBudgetCad <= 0) return { swapped: 0, saved: 0 };
        const dates = weekDates(weekStart);
        const goalKind = tableGoalOf(body, seats ?? []);
        const pool = allowedPool(unlocked, prefs, allergies, hidden, goalKind)
          .filter((r) => !seatAvoidsName(r.name, seats ?? []))
          .sort((a, b) => plateCost(a, household) - plateCost(b, household));
        const dinnerCost = (m: PlannedMeal) => {
          const rec = resolveMeal(m).recipe;
          return rec ? plateCost(rec, household) : 0;
        };
        const weekCost = (list: PlannedMeal[]) =>
          list
            .filter((m) => dates.includes(m.date) && m.slot === "dinner" && !m.skip)
            .reduce((sum, m) => sum + dinnerCost(m), 0);
        const start = weekCost(meals);
        if (start <= weekBudgetCad) return { swapped: 0, saved: 0 };
        let next = meals.slice();
        let swapped = 0;
        const snapshot = meals;
        while (weekCost(next) > weekBudgetCad && swapped < 6) {
          const dinners = next
            .filter((m) => dates.includes(m.date) && m.slot === "dinner" && !m.skip && !cookedDates.includes(m.date))
            .sort((a, b) => dinnerCost(b) - dinnerCost(a));
          const priciest = dinners[0];
          if (!priciest) break;
          const used = new Set(next.map((m) => m.recipeId).filter(Boolean) as string[]);
          const cheaper = pool.find((r) => !used.has(r.id) && plateCost(r, household) + 0.5 < dinnerCost(priciest));
          if (!cheaper) break;
          next = next.map((m) =>
            m.id === priciest.id ? { ...m, recipeId: cheaper.id, custom: undefined, auto: true } : m,
          );
          swapped += 1;
        }
        const saved = Math.max(0, Math.round(start - weekCost(next)));
        if (swapped > 0) set({ meals: next, undoMeals: snapshot });
        return { swapped, saved };
      },
      rememberBag: (rows) =>
        set((s) => {
          const bagMemory = { ...s.bagMemory };
          for (const row of rows) {
            const key = row.name.trim().toLowerCase();
            if (!key || !row.pickName) continue;
            bagMemory[key] = { pickName: row.pickName, packs: row.packs, pickCad: row.pickCad };
          }
          return { bagMemory };
        }),
      platePostLift: () => {
        const today = isoDate();
        const { meals, unlocked, prefs, allergies, hidden, body, seats, snacks, portionMultByDate, cookedDates, workouts, stepsByDate, goal } =
          get();
        ...
        const pool = allowedPool(unlocked, prefs, allergies, hidden, goalKind)
          .filter(
            (r) =>
              !seatAvoidsName(r.name, seats ?? []) &&
              isHighProtein(r) &&
              r.minutes <= 35 &&
              (r.nutrition?.protein ?? 0) >= 28,
          )
          .sort((a, b) => (b.nutrition?.protein ?? 0) - (a.nutrition?.protein ?? 0));
        const used = new Set(meals.filter((m) => m.recipeId).map((m) => m.recipeId as string));
        const need = Math.max(20, remaining.protein);
        const pick =
          pool
            .filter((r) => !used.has(r.id))
            .sort(
              (a, b) =>
                Math.abs((a.nutrition?.protein ?? 0) - need) - Math.abs((b.nutrition?.protein ?? 0) - need),
            )[0] ?? pool[0];
        if (!pick) return null;
      ...
      clipIntoTonight: (text) => {
        const raw = text.trim();
        if (!raw) return null;
        if (!get().consumeLookup()) return null;
        const lines = raw
          .split(/\n/)
          .map((line) => line.trim())
          .filter(Boolean);
        const name = lines[0]!.replace(/^#+\s*/, "").slice(0, 80);
        const rest = lines.slice(1);
        const ingredients: Ingredient[] = rest
          .filter((line) => !/^(step|direction|method|instruction|preheat)/i.test(line))
          .slice(0, 20)
          .map((line) => {
            const m = line.replace(/^[-*•]\s*/, "").match(/^([\d./]+)\s*([a-zA-Z]+)?\s+(.+)$/);
            if (m) {
              const qty = Number(m[1]!.replace(",", ".")) || 1;
              return { name: m[3]!.slice(0, 60), qty, unit: (m[2] ?? "").slice(0, 12), aisle: "Other" as Aisle };
            }
            return { name: line.replace(/^[-*•]\s*/, "").slice(0, 60), qty: 1, unit: "", aisle: "Other" as Aisle };
          });
        get().assignCustom(isoDate(), "dinner", {
          id: `clip-${Date.now()}`,
          name,
          minutes: 30,
          notes: rest.join("\n").slice(0, 800),
      ...
      assignCook: (date, name) =>
        set((s) => ({
          cookByDate: { ...s.cookByDate, [date]: name.trim().slice(0, 40) },
        })),
      setHandsFreeCook: (on) => set({ handsFreeCook: on }),
      toggleAlwaysHave: (name) => {
        const key = name.trim();
        if (!key) return;
        const s = get();
        const on = isAlwaysHave(key, s.alwaysHave);
        if (on) {
          set({
            alwaysHave: s.alwaysHave.filter(
              (n) => n.toLowerCase() !== key.toLowerCase() && !isAlwaysHave(n, [key]),
            ),
            pantry: s.pantry.filter(
              (p) => p.name.toLowerCase() !== key.toLowerCase() && !isAlwaysHave(p.name, [key]),
            ),
          });
          return;
        }
        set((prev) => ({
          alwaysHave: prev.alwaysHave.some((n) => n.toLowerCase() === key.toLowerCase())
            ? prev.alwaysHave
            : [...prev.alwaysHave, key.toLowerCase()],
          pantry: prev.pantry.some((p) => p.name.toLowerCase() === key.toLowerCase())
            ? prev.pantry
      ...
      earnWatchPlate: () => {
        const today = isoDate();
        const s = get();
        if (s.lastWatchDate === today) return false;
        const week = mondayOf();
        const bonus = (s.chefBonusWeek === week ? s.chefBonus : 0) + 1;
        set({ lastWatchDate: today, chefBonus: bonus, chefBonusWeek: week });
        get().awardXp(2);
        return true;
      },
      claimInvitePlate: () => {
        const s = get();
        if (s.inviteClaimed) return false;
        const week = mondayOf();
        const bonus = (s.chefBonusWeek === week ? s.chefBonus : 0) + 1;
        set({ inviteClaimed: true, chefBonus: bonus, chefBonusWeek: week });
        return true;
      },
      claimGiftCode: (raw) => {
        const code = normalizeGiftCode(raw);
        if (!isGiftCode(code)) return false;
        const s = get();
        if (s.claimedGifts.includes(code)) return false;
        const until = giftUntilFrom(isoDate(), 30);
        const next = s.giftTableUntil && s.giftTableUntil > until ? s.giftTableUntil : until;
        set({
          giftTableUntil: next,
          claimedGifts: [...s.claimedGifts, code],
        });
        return true;
      },
      seatCap: () => seatCap(get().unlocked),
      hideRecipe: (recipeId) =>
        set((s) => ({
      ...
      undoFill: () => {
        ...
        set({ meals: snap, undoMeals: null, planUpdates: [] });
        return true;
      },
      addPantry: (name) => {
        const trimmed = name.trim();
      ...
      stashCheckedToPantry: () => {
        const { weekStart, meals, extraGrocery, pantry, checked, household, alwaysHave } = get();
        const lines = groceryForWeek(meals, weekStart, extraGrocery, pantry, household, { alwaysHave });
        let n = 0;
      ...
      unlock: (id) => {
        ...
        set((s) => {
          if (id === "plates-15" || id === "plates-40" || id === "sos-3") {
            const week = mondayOf();
            const add = id === "plates-15" ? CHEF_PACK_15 : id === "sos-3" ? 3 : CHEF_PACK_40;
          ...
          if (id === "gift-table") {
            ...
            return {
              lastGiftCode: code,
              giftCodes: [...s.giftCodes, code],
              unlocked: s.unlocked.includes(id) ? s.unlocked : [...s.unlocked, id],
            };
          }
          if (id === "tip-flour" || id === "tip-butter") {
            const week = mondayOf();
            const add = id === "tip-flour" ? 1 : 3;
            const bonus = (s.chefBonusWeek === week ? s.chefBonus : 0) + add;
            return { chefBonus: bonus, chefBonusWeek: week, unlocked: s.unlocked.includes(id) ? s.unlocked : [...s.unlocked, id] };
          }
          if (id === "streak-save") {
          ...
          const extra: AddonId[] = [];
          if (id === "kitchen-table" || id === "founder" || id === "table-year") {
            extra.push("chef-plus", "family", "kitchen-table");
          }
          if (id === "trainer") extra.push("chef-plus", "family");
          if (id === "team-kitchen") extra.push("family");
        ...
        if ((id === "family" || id === "kitchen-table" || id === "founder" || id === "table-year") && !already)
          get().awardXp(15);
      },
      hasAddon: (id) => isUnlocked(get().unlocked, id, { giftUntil: get().giftTableUntil }),
      finishWalkthrough: () => set({ walkthroughDone: true, walkthroughOpen: false }),
      resetWalkthrough: () => set({ walkthroughDone: false, walkthroughOpen: true }),
      markPlanUpdatesRead: () => set({ planUpdates: [] }),
      favoriteStore: (store) => set({ favStore: store }),
      setSteps: (date, steps) =>
      ...
      syncFitness: ({ live } = {}) => {
        ...
        const eaten = nutritionForDate(now.meals, today, now.snacks, now.portionMultByDate, now.cookedDates);
      ...
      addSeat: (name, goalKind) => {
        ...
        if (!trimmed) return;
        set((s) => {
          if ((s.seats ?? []).length >= seatCap(s.unlocked)) return s;
          return {
      ...
      markSession: (id, status) => {
        ...
        if (status === "skipped") pingCoach(get, "skippedWorkout");
        else if (status === "missed") pingCoach(get, "missedWorkout");
        else if (status === "done") pingCoach(get, "loggedLift");
      ...
      replateFromFuel: (onlyDates, opts) => {
        const program = get().ensureProgram();
        const {
          weekStart,
          meals,
        ...
        for (const date of dates) {
          ...
          const fromName = existing ? resolveMeal(existing).title : "Nothing plated";
          used.add(pick.id);
          ...
          updates.push({
            ...
            fromName,
            toName: pick.name,
            why: plateChangeWhy(plateChangeKind(session?.status, sessionAfterLift(session, today))),
          });
        }
        ...
        set((s) => ({
          meals: [...rest, ...added],
          undoMeals: snapshot,
          planUpdates: notify ? [...updates, ...s.planUpdates].slice(0, 12) : s.planUpdates,
        }));
        const latest = updates[0];
        if (notify && latest && get().focusLock <= 0) {
          toast(`${latest.toName}. ${latest.why}`);
          if (get().notifyPrefs.meals) pushNote("Dinner changed", `${latest.toName}. ${latest.why}`);
        }
      ...
      kitchenPayload: () => {
        ...
        return {
          ...
          favMoves: s.favMoves,
          leftoverVault: s.leftoverVault,
          weekBudgetCad: s.weekBudgetCad,
          bagMemory: s.bagMemory,
          cookByDate: s.cookByDate,
          handsFreeCook: s.handsFreeCook,
          alwaysHave: s.alwaysHave,
          giftTableUntil: s.giftTableUntil,
          lastGiftCode: s.lastGiftCode,
          giftCodes: s.giftCodes,
          claimedGifts: s.claimedGifts,
      ...
      hydrateFromCloud: (payload) => {
        ...
        set({
          ...
          ...(Array.isArray(payload.planUpdates) ? { planUpdates: payload.planUpdates as KitchenUpdate[] } : {}),
          ...(payload.favStore && typeof payload.favStore === "object" ? { favStore: payload.favStore as NearbyStore } : {}),
          ...
          ...(payload.coach && typeof payload.coach === "object" ? { coach: normalizeCoach(payload.coach) } : {}),
          ...(typeof payload.weekBudgetCad === "number" ? { weekBudgetCad: payload.weekBudgetCad } : {}),
          ...(payload.bagMemory && typeof payload.bagMemory === "object"
            ? { bagMemory: payload.bagMemory as SpoonfulState["bagMemory"] }
            : {}),
          ...(payload.cookByDate && typeof payload.cookByDate === "object"
            ? { cookByDate: payload.cookByDate as Record<string, string> }
            : {}),
          ...(typeof payload.handsFreeCook === "boolean" ? { handsFreeCook: payload.handsFreeCook } : {}),
          ...(Array.isArray(payload.alwaysHave)
            ? { alwaysHave: migrateAlwaysHave(payload.alwaysHave, Array.isArray(payload.pantry) ? (payload.pantry as PantryItem[]) : []) }
            : {}),
          ...(typeof payload.giftTableUntil === "string" ? { giftTableUntil: payload.giftTableUntil } : {}),
          ...(typeof payload.lastGiftCode === "string" ? { lastGiftCode: payload.lastGiftCode } : {}),
          ...(Array.isArray(payload.giftCodes) ? { giftCodes: payload.giftCodes as string[] } : {}),
          ...(Array.isArray(payload.claimedGifts) ? { claimedGifts: payload.claimedGifts as string[] } : {}),
          ...(Array.isArray(payload.leftoverVault)
            ? { leftoverVault: payload.leftoverVault as SpoonfulState["leftoverVault"] }
            : {}),
        });
      },
      logWater: (ml) => {
      ...
      markCooked: (date) => {
        ...
        if (!already) {
          get().awardXp(25, "cooked");
          pingCoach(get, "cookedDinner");
        }
        ...
        const eatenElse = nutritionForDateExcluding(s.meals, date, s.snacks, dinner.id, s.cookedDates);
        const dayWork = expectedWorkoutsForDate({
          date,
          today: isoDate(),
      ...
      saveLeftovers: (fromDate) => {
        ...
        set((s) => ({
          leftoverVault: [
            { id: `vault-${fromDate}`, fromDate, title: resolved.title },
            ...s.leftoverVault.filter((row) => row.fromDate !== fromDate),
          ].slice(0, 8),
        }));
        return true;
      },
      addSnack: ({ date, name, nutrition, unknownMacros }) =>
        set((s) => ({
          snacks: [...s.snacks, { id: uid(), date, name, nutrition, ...(unknownMacros ? { unknownMacros: true } : {}) }],
        })),
      ...
      lookupRemaining: () => {
        ...
        const s = get();
        const cap = isUnlocked(s.unlocked, "chef-plus") ? 20 : LOOKUP_FREE_WEEK;
        return Math.max(0, cap - s.lookupCount);
      },
      ...
      setCoach: (patch) => set((s) => ({ coach: normalizeCoach({ ...s.coach, ...patch }) })),
      setTrainHud: (mode) => set({ trainHud: mode === "basic" ? "basic" : "advanced" }),
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        return {
          ...
          coach: normalizeCoach(p.coach ?? current.coach),
          trainHud: p.trainHud === "basic" ? "basic" : "advanced",
          ...
          alwaysHave: migrateAlwaysHave(p.alwaysHave, Array.isArray(p.pantry) ? p.pantry : current.pantry),
          focusLock: 0,
          walkthroughOpen: false,
        };
      },
...
if (typeof window !== "undefined") {
  try {
    ...
    if (raw) {
      const parsed = JSON.parse(raw) as { state?: { theme?: unknown; nextGen?: unknown } };
      const t = parsed?.state?.theme;
      const nextGen = parsed?.state?.nextGen;
      const patch: { theme?: ThemeId; nextGen?: boolean } = {};
      if (isThemeId(t)) patch.theme = t;
      if (typeof nextGen === "boolean") patch.nextGen = nextGen;
      if (patch.theme || patch.nextGen !== undefined) useSpoonful.setState(patch);
    }
  } catch {
    /* private mode */
  }
...
export function nutritionForDate(
  ...
  const fromMeals = mealsCountTowardFuel(date, cookedDates)
    ? meals
...
function migrateAlwaysHave(saved: unknown, pantry: PantryItem[]): string[] {
  if (!Array.isArray(saved)) return [];
  const list = saved.filter((x): x is string => typeof x === "string");
  const pantryHasBasics = pantry.some((p) => isBasicStaple(p.name));
  if (list.length > 0 && !pantryHasBasics) return [];
  return list;
}

function pantryHit(name: string, pantry: PantryItem[]): boolean {
  return pantry.some((p) => {
    if (isBasicStaple(p.name) || isBasicStaple(name)) return isAlwaysHave(name, [p.name]);
...
export function groceryForWeek(
  ...
  pantry: PantryItem[],
  household = 4,
  opts?: { scope?: ShopScope; date?: string; alwaysHave?: string[] },
): GroceryLine[] {
  ...
  const add = (name: string, qty: number, unit: string, aisle: Aisle, dish?: string) => {
    ...
    map.set(key, {
      ...
      unit,
      aisle,
      fromPantry: pantryHit(name, pantry) || isAlwaysHave(name, opts?.alwaysHave ?? []),
      dishes: dish ? [dish] : [],
    });
  };
  ...
  for (const meal of meals) {
    ...
    for (const ing of ingredients) {
      add(ing.name, scaleQty(ing.qty, household, servings), ing.unit, ing.aisle, resolved.title);
    }
  }

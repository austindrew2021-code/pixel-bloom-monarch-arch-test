...
export function programHint(kind: GoalKind | string | undefined): string {
  ...
  if (id === "lean") return "Five lift days. Miss one and a rest day becomes a catch-up workout; dinner still follows the work.";
  ...
  return {
    ...
    sessions: week.sessions.map((s) => {
      ...
      return {
        ...
        why: `Catch-up for a missed workout. Big lifts today. Dinner follows this workout, not the rest day.`,
        makeupOf: source.id,

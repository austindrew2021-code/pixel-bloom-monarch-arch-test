...
test("a missed lift schedules a makeup on the next rest day", () => {
  ...
  assert.ok(makeup!.name.toLowerCase().includes("catch-up"));
  assert.ok(!/makeup/i.test(makeup!.name));
  assert.ok(!/compounds/i.test(makeup!.why));
});
...
test("Sunday on a Cut week turns rest into a makeup for missed lifts", () => {
  ...
  assert.ok(sun.name.toLowerCase().includes("catch-up"));
});

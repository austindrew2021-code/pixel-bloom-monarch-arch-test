...
async function ensurePlayer(sql: Sql, userId: string): Promise<PlayerRow> {
 ...
 */
async function ensureSeason(sql: Sql, now = new Date()): Promise<SeasonRow> {
...
async function readServerSeed(sql: Sql, seasonId: string): Promise<string> {
...
async function readAllowance(sql: Sql, userId: string, day: string): Promise<AllowanceRow> {
...
async function readSeed(sql: Sql, userId: string, season: string) {
...
async function readScore(sql: Sql, userId: string, season: string) {
...
export const grantAdDrops = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const row: AllowanceRow | null = await claimAdBonus(sql, context.userId, season.id, day);
    if (!row) {
